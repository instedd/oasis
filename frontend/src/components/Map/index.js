...
import styles2 from "routes/Dashboard/styles.module.css";
...
import { fields, initialFieldsState } from "routes/Dashboard/fields";
...
function Map(props, { draggable = true }) {
...
/**
 * From this point is the Dashboard component and
 * its helpers
 * 
 */
...
function Dashboard(props, { draggableMapRoutes = [] }) {
  ...
  const actions = [
    ...
    },
    {
      ...
      href: paths.onboard,
      state: { onboard: false },
      classes: classNames("MuiFab-extended assessment", styles2.assessment),
    },
    {
      name: "EDIT MY PROFILE",
      href: paths.criticalQuestions,
      classes: classNames("MuiFab-extended", styles2.profile),
    },
  ];

  const userStatus = () => (
    <div
      className={classNames(styles2.statusList)}
      style={{ textAlign: "left" }}
    >
      <div className={classNames("row", styles2.statusItem)}>
        <span
          className={styles2.dot}
  ...
  const informationHeader = () => (
    <div className={classNames(styles2.box, styles2.top, styles2.header)}>
      {userStatus()}
      {/* {resources()} */}
    </div>
  );
  ...
  const profileBar = () => (
    <Grid
      container
      spacing={2}
      className={classNames(classes.profileBar, styles2.profileBar)}
    >
      <div
        style={{ display: errorMsg.display }}
        className={classNames(styles2.errorMsg)}
      >
      ...
      {status.type === LOADING || !story ? (
        ...
        <>
          <Map
            ...
            storyList={storyList}
          />
          <div className={classNames(styles2.right)}>{informationHeader()}</div>
          {barDisplay ? profileBar() : ""}
          <SpeedDial
            ariaLabel="Daily actions"
            className={classNames("speeddial", styles2.speeddial)}
            icon={<SpeedDialIcon />}
            onClick={handleClick}
            open={open}
          >
